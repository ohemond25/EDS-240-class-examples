...
#| echo: true
# line chart ----
ggplot(lyme_pop, aes(x = year, y = cases_per100k, group = state)) + # group by state to separate lines out
  geom_line() +
  geom_point()
```
...
ggplot(lyme_pop, aes(x = year, y = cases_per100k, group = state, color = state)) +
  geom_line() +
  # highlight states when cases per 100k has a value over 100
  gghighlight(max(cases_per100k) > 100)
```

### Note 1: It's okay to cut the y-axis

-   line plots encode data by position and not length. if including 0 makes your data too squished / not understandable, then cut it out. or if relationship to 0 is insignificant, then don't include it! (ex: body temp in degrees F)
...
- aspect ratio can influence how we perceive slope, manipulating aspect ratio can manipulate your viewers
...
lyme_pop |> 
  ...
  ggplot(aes(x = year, y = cases_per100k, group = state)) +
  geom_line() +
  # visualize aspect ratio by giving 1-unit breaks to the y axis
...
lyme_pop |> 
  ...
  ggplot(aes(x = year, y = cases_per100k, group = state)) +
  geom_line() +
  # want small ratio to spread out plot in x direction 
...
- show evolution of the whole + relative proportions of each group (order matters a lot for how this looks!)
- NOT good for showing the evolution of groups INDIVIDUALLY
...
- plot percentage contributions rather than absolute numbers on y-axis (can make interpretation easier)
