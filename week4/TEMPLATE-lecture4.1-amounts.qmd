...
#..........................load packages.........................
library(tidyverse)
library(scales) # helps with labels and readability
...
jobs_clean <- jobs |> 
  ...
  # make occupation a factor (for reordering groups in our plot) ----
      # `factor()` can be used this same way, but also has an additional argument to specify levels (not needed in this example)
...
Bar charts and lollipop charts are mostly interchangeable. Best for visualizing amounts compared across a single categorical variable.
...
# bar chart ----
jobs_clean %>% 
  # select just 2016 observations
  filter(year == 2016) %>% 
  # slice to take the top 10 observations (n), based upon total_earnings amount
  slice_max(order_by = total_earnings, n = 10) %>% 
  # reorder occupations by total_earnings column using fct_reorder
  mutate(occupation = fct_reorder(occupation, total_earnings)) %>% 
  
  ggplot(aes(x = occupation, y = total_earnings)) +
  
  geom_col() +
  # add labels of total_earnings to bars
     # need to use `dollar()` within geom_text(), instead of `label_currency()` (which superceded it)
  geom_text(aes(label = dollar(total_earnings)),
            hjust = 1.2, color = "white") +
  # modify how earnings appear. refer back to original aes() mappings to identify axes
     # show 100000 as $100k using `label_currency()`
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k")) +
  # flip axes so that labels are easier to read
  coord_flip()
```
...
jobs_clean |> 
  ...
  mutate(occupation = fct_reorder(occupation, total_earnings)) |> 
  ggplot(aes(x = occupation, y = total_earnings)) +
  # DIY lollipop chart using geom_point and geom_linerange
  geom_point() +
  geom_linerange(aes(ymin = 0, ymax = total_earnings)) +
  # add labels to lollipops
  geom_text(aes(label = dollar(total_earnings)),
            hjust = -0.2, vjust = 0.5, color = "black") +
  # if you wanted to round, use accuracy argument within label_currency
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k"),
                     # force axis limits if we want to fit in our added labels
                     limits = c(0, 250000)) +
  coord_flip()
```
...
- use `geom_col()` when you have data that's already summarized (data is already calculated in your data frame)
...
- use `geom_bar()` when you need ggplot to count up the number of rows for you (require a calculation to be done)
...
ggplot(jobs_clean, aes(x = major_category)) +
  geom_bar() + 
  coord_flip()
# gives count of number of observations in each major_category group
```
...
jobs_clean |> 
  ...
  # pivot longer -> single column that represents all the earning values, another column that represents group (female or male)
  pivot_longer(cols = c(total_earnings_female, total_earnings_male),
               names_to = "group", values_to = "earnings_by_group") %>% 
  # create new column with just the "sex" information. use str_remove to remove everything besides "female" and "male"
  mutate(sex = str_remove(string = group, pattern = "total_earnings_")) %>% 
  ggplot(aes(x = occupation, y = earnings_by_group, fill = sex)) +
  # create side by side columns using position_dodge() (function gives opportunity to use more arguments)
...
jobs_clean |>
  ...
  # specify width of dodge (required when you're working with lines and points)
  geom_point(position = position_dodge(width = 0.5)) +
  # specify width of dodge (make it the same as points so that things stay aligned)
    # we MUST set the linerange to begin at 0! can't cut the axis on plots like this, they lose their meaning
...
analysts <- jobs_clean |> 
  # extract any occuptions that have the word analyst in them
...
ggplot(heatmap_order, aes(x = year, y = occupation, fill = total_earnings)) +
  geom_tile() +
  # update heading for fill legend
  labs(fill = "Total earnings") +
  # set aspect ratio to force tiles to be squares
  coord_fixed() +
  # add colorblind friendly palette, and change format of earnings #s
  scale_fill_viridis_c(labels = label_currency(scale = 0.001, suffix = "k")) +
  # wrap long labels
  scale_y_discrete(labels = label_wrap(30)) +
  # adjust legend dimensions and size, and move title to the top of the bar
...
# dumbbell plot ----
ggplot(subset_jobs) +
  # map aesthetics differently for each layer
  # draw line first so that it falls beneath points
