...
# scatter + rug plot ----
p1 +
  # add rug plot to help visualize the distribution of data
  geom_rug()
```
...
# add marginal density plot ----
  # use arguments to create different density curves for each site (group)
...
stream_chem_data |> 
  ...
  ggplot(aes(x = DOC, y = pH)) + 
  geom_point(alpha = 0.5) +
  # force fit a linear model to the data
    # be cautious doing this! we haven't actually run any of the needed analysis
...
stream_chem_data |> 
  ...
  geom_point(alpha = 0.5) +
  # adjust size range of bubbles using scale_size. scales the AREA of the bubbles
...
# contour plot ----
ggplot(stream_chem_data, aes(x = SO4, y = pH)) +
  # shows % of points that fall into the differently demarked regions
...
#  ----
ggplot(stream_chem_data, aes(x = SO4, y = pH)) +
  # breaks data into bins based on density
  geom_hdr()
```
