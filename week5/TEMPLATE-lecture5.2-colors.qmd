...
cat_color_plot +
  # by default, applies 'viridis' scale.
  # viridis_d is for categorical
  scale_color_viridis_d()

# continuous color scales ----
con_color_plot +
  # viridis_c is for continuous
  # supply palette name or letter to 'option' to use something other than default
  # can flip scale direction using 'direction' command
...
# for qualitative (categorical) palettes ----
cat_color_plot +
  # automaticaly takes the most highly contrasting colors from your selected palette
  scale_color_brewer(palette = "Dark2")

# for unclassed continuous color scales ----
con_color_plot +
  # notice the different function here
  scale_color_distiller(palette = "BuPu")

# for classed (binned) continuous color scales ----
con_color_plot +
  # notice the third function here
  scale_color_fermenter(palette = "YlGnBu")

# brewer, distiller, fermenter -- all beer brewing related
```
...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, fill = body_mass_g)) +
  # change shape for points that has both fill and outline
  geom_point(size = 4, alpha = 0.8, shape = 21, color = "#484848") +
  scale_fill_distiller(palette = "BuPu")
```

- use redundant mapping (colors & shapes) whenever possible (important for accessibility)
...
Package that provides a common interface for accessing near-comprehensive list of color palettes, across various packages
...
# helpful if you know what you're looking for, not really otherwise
...
cat_color_plot +
  # calecopal is california-inspired color palettes! name the package then the palette
...
con_color_plot +
  # use same palette as above, but break into discrete bins
...
con_color_plot +
  # argument is "colors" here unlike "values" above
...
ggplot(na.omit(penguins), aes(x = species, fill = sex)) +
  geom_bar() +
  # adjust chroma / saturation (intesity level of color)
  scale_fill_hue(c = 70)
```
...
ggplot(na.omit(penguins), aes(x = bill_length_mm, y = bill_depth_mm, color = sex)) +
  geom_point() +
  # reducing luminence results in darker colors
...
#| warning: false
# create palette ----
# creating named palette creates consistency across plots by always showing the same color for the same name
...
- Ex 1: color points based on value in dataframe
...
# color using the color variable
ggplot(penguins_ex1, aes(x = bill_length_mm, y = bill_depth_mm, color = my_color)) +
  geom_point(size = 4, alpha = 0.8) +
  # extract HEX codes from dateframe, and map to the data points
...
# when plotted, this highlights the points in red that were greater than the certain criteria
ggplot(penguins_ex2, aes(x = bill_length_mm, y = bill_depth_mm, color = my_color)) +
  geom_point(size = 4, alpha = 0.8) +
  # if you want a legend, have to specify
  scale_color_identity(guide = "legend",
                       name = "Body mass (g)",
                       labels = c("> 6,000", "<= 6,000"))
```
