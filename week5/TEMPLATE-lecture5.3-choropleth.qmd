...
my_brew_palette11 <- brewer.pal(n = 11, name = 'BrBG')
```
...
base_map +
  # apply palette, add percent labels, update # of breaks, center values around 0 using rescale
  scale_fill_gradientn(colors = my_brew_palette11,
                       labels = label_percent(scale = 1),
                       breaks = breaks_width(width = 10),
                       values = rescale(x = c(-40, 0, 20))) +
  # stretch out scale bar
...
# called 10 instead of 11 colors, to get rid of the neutral middle color
...
base_map +
  scale_fill_stepsn(colors = my_brew_palette10,
                    ...
                    values = rescale(x = c(-40, 0, 20))) +
  guides(fill = guide_colorsteps(barwidth = 25, barheight = 0.75))
# R will interpolate and create new bins to fill in the extra colors
```
